...
export default function Quiz() {
  ...
  const nextBtn = () => {
    if (currentStep < allQuestions.length) {
      ...
      return (
        <QuizBtn text={"Finish"} onClick={() => setShowConfirmModal(true)} />
      );
    }
  };
          ...
          <Stack spacing={8} mx={"auto"} w={"768px"}>
            <Stack align={"center"} direction={"row"} justify={"space-between"}>
              <Heading fontSize={"2xl"}>
                Question {currentStep} out of {allQuestions.length}
              </Heading>
              <Countdown
            ...
            >
                ...
                <RadioGroup onChange={handleChange} value={currentAns}>
                  <Stack direction="column" spacing={4}>
                    {allQuestions[currentQuestion]?.options.map((opt, i) => (
                      <Radio
                        value={opt}
                        key={`${allQuestions[currentQuestion]?.text}-${opt}`}
                      >
                        {opt}
                      </Radio>
                    ))}
              ...
              {allQuestions[currentQuestion]?.type === "Fill" && (
                <Box padding="10px">
                  {allQuestions[currentQuestion]?.dropdowns.map(
                    (dropdown, index) => (
                      <FormControl key={index} marginY="10px">
                        ...
                        <Select
                          ...
                          variant="filled"
                          onChange={(e) =>
                            handleFillChange(e.target.value, index)
                          }
                          value={currentAns ? currentAns["Option" + index] : ""}
                        >
                          {dropdown.options.map((option, i) => (
                            <option
                              key={`${allQuestions[currentQuestion]}-Option${index}-${option}`}
                              value={option}
                            >
                              {option}
                            </option>
...
Quiz.getLayout = function getLayout(page) {
  return <Layout showAppBar={false}>{page}</Layout>;
};
