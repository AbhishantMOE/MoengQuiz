      ...
      return (
        <Stack spacing={4} direction={"column"}>
          {response.options.map((opt, i) => (
            <OptionItem resp={response} text={opt} key={i} />
          ))}
        </Stack>
      );
        ...
        <Stack spacing={4} direction={"column"}>
          {response.dropdowns.map((dropdown, i) => (
            <Box key={i}>
              ...
              <Text
                color={
                  dropdown.correctAnswer === Object.values(response.selected)[i]
                    ? "green"
                    : "red"
                }
              >
  ...
  const respIcon = (resp) => {
    if (resp.type === "Hotspot") {
      ...
      if (
        ...
        return <Icon as={IoCheckmarkDoneOutline} w={4} h={5} color={"green"} />;
      } else if (String(resp?.selected) === null) {
        return <Icon as={IoWarningOutline} w={4} h={5} color={"goldenrod"} />;
      } else {
      ...
      if (
        userAnsArray.sort().join(",") === resp.correctAnswer.sort().join(",")
      ) {
        return <Icon as={IoCheckmarkDoneOutline} w={4} h={5} color={"green"} />;
      } else {
        return <Icon as={IoCloseOutline} w={4} h={5} color={"red"} />;
    ...
    <AccordionItem my={3}>
      {({ isExpanded }) => (
        <>
          ...
          <AccordionPanel pb={4}>{renderAnswerChoices()}</AccordionPanel>
        </>
      )}
    </AccordionItem>
  );
...
const OptionItem = ({ resp, text }) => {
  ...
  return (
    <Stack spacing={4} direction={"row"} alignItems={"center"}>
      <Icon as={IoDiscOutline} w={4} h={4} color={optionColor} />
      <Text color={optionColor}>{text}</Text>
